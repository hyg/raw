    ...
    for (var id in food) {
        if (food[id].name in emap) {
            ...
            for (var e in fooddata.element) {
                ...
                if(e=="碳水化合物") console.log(food[id].amount+food[id].unit+"\t"+food[id].name+nametab+"含有"+item.amount.toFixed(8)+item.unit+"。\t累计摄入："+etable[e].amount.toFixed(8)+item.unit+"\t累计nrv:"+etable[e].nrv.toFixed(2)+"%");
            }
            delete food[id];
        } else {
            //newfood.push(item);
