    ...
    for (var id in food) {
        if (food[id].name in emap) {
            ...
            for (var e in fooddata.element) {
                ...
                //if(e=="热量") console.log(food[id].amount+food[id].unit+"的"+food[id].name+"\t含有"+item.amount.toFixed(10)+item.unit+"。\t累计摄入："+etable[e].amount.toFixed(10)+"\t累计nrv:"+etable[e].nrv.toFixed(2));
            }
            delete food[id];
        } else {
            //newfood.push(item);
